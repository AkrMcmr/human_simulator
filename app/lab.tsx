...
import { createSimulation, observeSimulation, stepSimulation, archiveRun, restoreRun, runExperiment, type Frame, type Provenance } from "@/packages/simulation/src/index.ts";
...
export default function Lab({ provenance }: { provenance: Provenance }) {
  ...
  async function importFile(file?: File) {
    ...
    try {
      ...
      setDraft({ ...structuredClone(DEFAULT_SETTINGS), seed: restored.config.seed, horizon: restored.config.horizon, initialDistance: Math.hypot(a.position.x - b.position.x, a.position.y - b.position.y), ambientCold: restored.state.world.parameters.ambientCold, soundEnabled: restored.state.world.parameters.soundEnabled, resourceLayout: restored.config.resources?.some(r => r.id === 'food-center') ? 'shared' : 'separate', a: { ...DEFAULT_PARAMETERS, ...a.parameters }, b: { ...DEFAULT_PARAMETERS, ...b.parameters } });
  ...
  return <div className="lab-shell"><header className="topbar"><Link className="brand" href="/"><span className="brand-mark"><Activity size={22} /></span><span>HUMAN WORLD LAB<small>人間モデル実験室</small></span></Link><span className="version-pill"><i />探索モデル v0.1</span></header>
    ...
    {tab === 'notes' && <div className="panel notes-panel"><p className="eyebrow">A MODEL THAT CAN EVOLVE</p><h2>固定するのは、問いと境界。</h2><p>知覚 → 内部状態の更新 → 行動選択 → 世界の変化 → フィードバック。この循環を、観察可能で差し替え可能なモジュールとして実装しています。実行時にLLMは使いません。</p><h3>生まれた時点で与えているもの</h3><p>成人相当を想定した移動・発声の能力、空腹・寒さ・疲労への対処、同種らしさを見分ける能力。好奇心・警戒傾向・学習率と効用式は、実装上の仮定です。</p><h3>今、学習するもの</h3><p>自分の行動の後に相手との距離がどう変化したか、近くにいた相手と危害が結びついたか、聞いた音にどんな特徴のまとまりがあるか。相手の思考や意図は渡しません。これらは相関の学習で、意図の理解ではありません。</p><h3>まだ入っていないもの</h3><p>音と対象・目的の対応、共有語彙、文法、社会的規範、愛着、性欲、生殖、育児、世代交代。発声が増えても「言語が生まれた」とは判定しません。</p><h3>次に改善するための約束</h3><ul><li>人間・世界・実行エンジン・実験・表示の責務を分離する。</li><li>モデルと実験条件の版を記録し、同条件で比較する。</li><li>仮定、学習の結果、実証済みの性質を区別する。</li><li>単位は抽象的なステップと距離。現実の秒やメートルに換算しない。</li></ul><h3>調査の出発点</h3><p className="reference-links"><a href="https://act-r.psy.cmu.edu/about/" target="_blank" rel="noreferrer">ACT-R</a><a href="https://www.pyoudeyer.com/connectionScienceOudeyer05.pdf" target="_blank" rel="noreferrer">音声の自己組織化</a><a href="https://www.jasss.org/22/3/6.html" target="_blank" rel="noreferrer">モデルの検証</a></p><p className="caption">比較・調査する候補です。この初版が各理論を再現しているという意味ではありません。</p><div className="model-note">探索モデル v0.1 · 計算の基盤を検証する段階です。高精度な人間モデルとしての校正・実証はこれから行います。</div></div>}
