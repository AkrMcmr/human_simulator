...
import { VERSIONS, versionsFor } from "../packages/simulation/src/index.ts";
...
const baselineVersions = versionsFor("human-0.1.0");
const baseline: Report = { format: "human-world-lab/evaluation", schemaVersion: 1, provenance: { ...provenance, versions: baselineVersions }, evaluation: runEvaluation({}, baselineModel) };
...
const output = { format: "human-world-lab/policy-study", schemaVersion: 1, protocol, provenance, variants: { baseline: baselineVersions.human, candidate: PREDICTIVE_POLICY_VERSION, ablated: "v0.1.0 with zero outcome bonus" }, policy, core: { baseline, candidate, comparison: coreComparison }, gatesMet, decision: "requires-review-not-default-promotion" };
mkdirSync(dirname(target), { recursive: true }); writeFileSync(target, JSON.stringify(output, null, 2) + "\n");
const lines = ["# 予測を行動選択へ接続する比較実験", "", `- 条件群: ${split}`, `- ソース: ${provenance.commit}`, `- 未コミット変更: ${provenance.dirty}`, `- モデルコードSHA256: ${provenance.modelHash}`, `- 評価器SHA256: ${provenance.evaluatorHash}`, `- 旧版: ${baselineVersions.human} / 候補: ${PREDICTIVE_POLICY_VERSION}`, `- シード: ${policy.seeds.join(", ")}`, "", "値はcandidate−baselineの最終48試行の距離変化。無関係条件は差の絶対値の負、null-bonusは候補効用寄与の最大絶対値の平均の負。全項目で高い方が良い方向。区間はシード単位のばらつきの目安で、人間データへの推論ではない。", "", "| 項目 | 平均 ± SD | 閾値 | 判定 |", "| --- | ---: | ---: | --- |"];
