...
const lines = ["# 自由worldでの最小合図の診断（signal-world-v1）", "", `- 条件群: ${split}`, `- ソース: ${provenance.commit}`, `- 未コミット変更: ${provenance.dirty}`, `- モデルコードSHA256: ${provenance.modelHash}`, `- 評価器SHA256: ${provenance.evaluatorHash}`, `- 環境SHA256: ${provenance.environmentHash}`, `- シード: ${partitions[0].seeds.join(", ")}`, `- 実行: ${protocol.horizon}ステップ、初期距離${protocol.initialDistance}u、資源${protocol.resourceLayout}、二人とも同じモデル。対照は音なしと、聞こえる音の特徴をでたらめに置き換える介入`, "", "値はシードごとの超過相互情報量（bit）または差の平均 ± 標本SD。高いほど仮説の方向。相関の強さであり、意味の理解や意図ではない。", ""];
