...
export type SenderOptions = { sender?: boolean; receiver?: boolean; stateCoupling?: number; soundOrienting?: DecideOptions["soundOrienting"] };
...
export function decideWithSenderOptions(previous: HumanState, observation: Observation, random: RandomSource, options: SenderOptions = {}) {
  ...
  const result = decideWithSoundOptions(human, observation, random, { association: options.receiver !== false, chooseSound: options.sender === false ? undefined : chooseSignalSound, stateCoupling: options.stateCoupling, soundOrienting: options.soundOrienting });
