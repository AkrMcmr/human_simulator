...
import type { DecideOptions, Estimate, HumanState, SoundChoice } from "./index.ts";
...
export type SenderOptions = { sender?: boolean; receiver?: boolean; stateCoupling?: number; soundOrienting?: DecideOptions["soundOrienting"]; satiationCall?: number; eatingCoupling?: number; /** Replaces the learned risk-based choice (0.10.0 imitation candidates); the sender still records what followed. */ chooseSound?: SoundChoice };
...
export function decideWithSenderOptions(previous: HumanState, observation: Observation, random: RandomSource, options: SenderOptions = {}) {
  ...
  const result = decideWithSoundOptions(human, observation, random, { association: options.receiver !== false, chooseSound: options.sender === false ? undefined : (options.chooseSound ?? chooseSignalSound), stateCoupling: options.stateCoupling, soundOrienting: options.soundOrienting, satiationCall: options.satiationCall, eatingCoupling: options.eatingCoupling });
