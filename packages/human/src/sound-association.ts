...
import type { DecideOptions, HumanState, OutcomeBonus, SoundChoice } from './index.ts';

export const SOUND_ASSOCIATION_VERSION = '0.4.0-experimental.1';
export type SoundOptions = { association?: boolean; classification?: boolean; attention?: boolean; policy?: boolean; chooseSound?: SoundChoice; stateCoupling?: number; soundOrienting?: DecideOptions["soundOrienting"]; warmthOrienting?: DecideOptions["warmthOrienting"]; satiationCall?: number; shelterCall?: number; eatingCoupling?: number; auditoryResolution?: number; callUrge?: number };
...
export function decideWithSoundOptions(previous:HumanState, observation:Observation, random:RandomSource, options:SoundOptions={}) {
  if(options.association===false) return options.chooseSound||options.stateCoupling||options.soundOrienting||options.warmthOrienting||options.satiationCall||options.shelterCall||options.eatingCoupling||options.auditoryResolution||options.callUrge ? decideWithOptions(previous,observation,random,{outcomeBonus:predictedSafety,chooseSound:options.chooseSound,stateCoupling:options.stateCoupling,soundOrienting:options.soundOrienting,warmthOrienting:options.warmthOrienting,satiationCall:options.satiationCall,shelterCall:options.shelterCall,eatingCoupling:options.eatingCoupling,auditoryResolution:options.auditoryResolution,callUrge:options.callUrge}) : decideHuman(previous,observation,random);
  ...
  const result=decideWithOptions(human,observation,random,{outcomeBonus:predictedSafety,signalBonus:bonus,
    auditoryClassification:options.classification,auditoryAttention:options.attention,chooseSound:options.chooseSound,stateCoupling:options.stateCoupling,soundOrienting:options.soundOrienting,warmthOrienting:options.warmthOrienting,satiationCall:options.satiationCall,shelterCall:options.shelterCall,eatingCoupling:options.eatingCoupling,auditoryResolution:options.auditoryResolution,callUrge:options.callUrge});
