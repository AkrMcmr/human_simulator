...
export type SoundOptions = { association?: boolean; classification?: boolean; attention?: boolean; policy?: boolean; chooseSound?: SoundChoice; stateCoupling?: number; soundOrienting?: DecideOptions["soundOrienting"]; satiationCall?: number; eatingCoupling?: number };
...
export function decideWithSoundOptions(previous:HumanState, observation:Observation, random:RandomSource, options:SoundOptions={}) {
  if(options.association===false) return options.chooseSound||options.stateCoupling||options.soundOrienting||options.satiationCall||options.eatingCoupling ? decideWithOptions(previous,observation,random,{outcomeBonus:predictedSafety,chooseSound:options.chooseSound,stateCoupling:options.stateCoupling,soundOrienting:options.soundOrienting,satiationCall:options.satiationCall,eatingCoupling:options.eatingCoupling}) : decideHuman(previous,observation,random);
  ...
  const result=decideWithOptions(human,observation,random,{outcomeBonus:predictedSafety,signalBonus:bonus,
    auditoryClassification:options.classification,auditoryAttention:options.attention,chooseSound:options.chooseSound,stateCoupling:options.stateCoupling,soundOrienting:options.soundOrienting,satiationCall:options.satiationCall,eatingCoupling:options.eatingCoupling});
