...
test("omitting the model selects the default, and the default archive records human 0.1.0", () => {
...
test("the candidate is selected only by name, and its archive names its own version", () => {
  const candidate = runExperiment(config("predictive-0.2.0-experimental.1"));
  const archive = archiveRun(config("predictive-0.2.0-experimental.1"), candidate.state, candidate.frames, provenance);
  assert.equal(archive.manifest.versions.human, "0.2.0-experimental.1");
  assert.notEqual(archive.manifest.versions.human, VERSIONS.human);
  assert.deepEqual(restoreRun(JSON.parse(JSON.stringify(archive))).state, candidate.state);
  assert.ok(candidate.frames.some(f => f.agents.some(a => a.trace && a.trace.scores.some(s => "predictedSafety" in s.terms))));
});
test("a candidate archive cannot be replayed as the default model or under another version", () => {
  const candidate = runExperiment(config("predictive-0.2.0-experimental.1"));
  const archive = archiveRun(config("predictive-0.2.0-experimental.1"), candidate.state, candidate.frames, provenance);
  ...
  const forgedVersion = JSON.parse(JSON.stringify(archive)); forgedVersion.manifest.versions.human = "0.1.0";
  assert.throws(() => restoreRun(forgedVersion), /読み込めない/);
  const legacy = JSON.parse(JSON.stringify(archive)); legacy.manifest.versions.simulation = "0.1.0";
  assert.throws(() => restoreRun(legacy), /読み込めない/);
});
...
test("the ablated control reproduces the default model's world, bodies, and actions", () => {
  const baseline = runExperiment(config(DEFAULT_MODEL_ID));
