...
1. M3の設計相談（[ROADMAP](ROADMAP.md)の「どこまで生得的か」）: 発し手の音の特徴が本人の内部状態（危険度など）に連動する初期能力を認めるか。認めるなら「状態連動の発声」を候補として`prospective`登録し、聞き手が学べる構造が生まれるかをsignal-world-v1で測る。認めないなら、発声分類の粒度（最大16、閾値0.14）と再利用率を変える候補を先に試す。
2. signal-world-v1の開発14001–14008・確認15001–15008は観察済み。次の候補比較には新しいシードを使う。
