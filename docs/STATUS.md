...
1. 情報の非対称がある課題（protocol `referential-v1`）の設計: 発し手だけが視認できる危害源、聞き手の視界外に散在する食料、課題に合わせた聞き手の反応の定義（発し手の方へ動く、食料の方へ動く）。パイロットで尺度を確認→閾値固定→`prospective`登録→既定・受信側候補・voice-state候補を比較。world側の変更（第三の個体や危害源）は`world`の版を上げ、両モデルを同じ世界で再評価する。
