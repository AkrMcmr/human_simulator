...
1. 受け手側の合図利用は成立（[判断0020](../research/decisions/0020-referential-v5.md)）。次は発し手側と慣習: 発し手が声の結果を学ぶと呼び声は減るが黙らない（[判断0021](../research/decisions/0021-caller-cost.md)。抑制0.27 / 0.26、閾値0.3未達。食べている状態はどの発声にも漏れるので、正直な合図は発し手が抑えても消えにくい）。次は発し手側に利益を与える世界（寒さの共有、判断0022）。音の目録が二人以上で収束する条件（模倣）、複数の指示対象（食料と暖かさ）での声の分化。
