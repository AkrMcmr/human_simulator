...
## 2026-09-20: referential-v2

食料を十分な量にし初回摂食までの時間を主要指標にしたreferential-v2で、既定・定位のみ・選択的・呼び声のみ・呼び声＋盲目・呼び声＋選択的の6モデルを開発20001–20008・確認21001–21008で測定。時間の2項目は定位のある5モデルが両群達成、種類依存は全モデル未達、副作用は候補により未達（[判断0015](../research/decisions/0015-referential-v2.md)）。今回の検証: `npm run test:model` 59/59、型検査・lint成功。
...
convention-v2の16シード再現（[判断0035](../research/decisions/0035-convention-replication.md)）は5項目を両群で達成。lexicon-v3・分離・中間の希少性・細かい聴覚（[判断0031](../research/decisions/0031-memory-credit.md)〜[0036](../research/decisions/0036-fine-hearing.md)）は暖かさ側が未達で、2文脈の系統は7ラウンドで閉じた。generations-v2（[判断0032](../research/decisions/0032-generations-v2.md)）は連続0.115 / 0.125で未達（漂いとして記録）。valence-v1（70001–70008 / 71001–71008、[判断0037](../research/decisions/0037-valence.md)）: 契約0.3.0（`poison`効果）・世界0.5.0（毒の食料）を追加、既定の世界は不変（保存結果と一致するテスト）。候補は良い声・悪い声の収束と任意性を両群達成、区別は0.283 / 0.213（閾値0.25、対照0.06未満）、毒の利益は後ろ3分の1で−0.03 / −0.04で未成立。世界0.6.0（`foodSpawn.lifetime`）のvalence-v2（72001–72008 / 73001–73008、[判断0038](../research/decisions/0038-valence-rot.md)）: 区別0.264 / 0.272で両群達成、毒の利益−0.32 / −0.08で未成立。味覚嫌悪の候補と私的嫌悪の対照のvalence-v3（74001–74008 / 75001–75008、[判断0039](../research/decisions/0039-taste-aversion.md)）: 毒の摂食は全条件で約75%減、悪い声の収束0.03 / 0.00、区別0.14 / 0.09で未成立。嫌悪の声の候補・聞かない対照・評価器の単位尺度（毒の利益0.3単位・種類依存0.15単位）のvalence-v4（76001–76008 / 77001–77008、[判断0040](../research/decisions/0040-disgust-call.md)）: 候補は開発2項目・確認6項目で未成立（声の洪水と餓死）。控えた嫌悪の声の候補・毒に当たる回数の尺度・毒3つに1つの世界のvalence-v5（パイロット78101–78104、78001–78008 / 79001–79008、[判断0041](../research/decisions/0041-restrained-disgust.md)）: 候補は開発1項目・確認4項目（毒に当たる回数の利益+0.63回）で未成立。対比較（`assessPaired`）・上限対照・間隔10の候補のvalence-v6（80001–80008 / 81001–81008、[判断0042](../research/decisions/0042-ceiling-and-paired.md)）: 候補は産出4項目を両群達成、対比較+0.5 / −0.9、上限対照の利益0.13 / 0.88で未成立。診断（`poisoningKinds`）で飢えと理解の不成立を分離。産出による理解の候補・非常時割合のvalence-v7（[判断0043](../research/decisions/0043-mirror-by-production.md)）: パイロット4回（82101–82134）で世界を量6・7か所に確定（非常時2%、上限対照の利益1.5回）。本番（82001–82008 / 83001–83008）で上限対照は1.75 / 2.0回を達成、候補experimental.6は産出崩壊（実装の欠陥）で未成立。修正版experimental.7のvalence-v8（84001–84008 / 85001–85008、[判断0044](../research/decisions/0044-mirror-corrected.md)）: 候補は開発3項目・確認2項目、対比較+0.1 / −0.6回で未成立。毒課題は8ラウンドで一区切り。所有者の選択で厳密な連合学習のvalence-v9（86001–86008 / 87001–87008、[判断0045](../research/decisions/0045-strict-association.md)）: 対比較0.0 / 0.1回で未成立（数え方の欠陥を診断）。訪問1回につき1回の連合のvalence-v10（パイロット88101–88114で9か所・毒2つに1つに確定、88001–88008 / 89001–89008、[判断0046](../research/decisions/0046-episode-association.md)）: 声を出す全モデルで後ろ3分の1の毒当たりが0〜0.75回に落ち、悪い声の出番が消える。対比較−0.25 / −1.25で未成立。世界0.7.0（毒の潜伏）と潜伏対応の候補でvalence-v11（[判断0047](../research/decisions/0047-latent-poison.md)）を登録したが、パイロット3回（90101–90124）で上限対照の基準を満たせず保留（候補未実行）。既定の世界は不変（保存結果と一致するテスト）。毒課題を閉じ（[判断0048](../research/decisions/0048-close-poison-line.md)）、観察画面に「二つの声」パネル、ダイジェストに発見13・14を追加（型検査・lint・`npm run test:observer` 9/9・検証ビルド成功）。世界0.8.0（捕食者）と襲撃の尺度でpredator-v1（[判断0049](../research/decisions/0049-predator-design.md)）を登録し、既定のパイロット4回（92101–92134）で世界の水準を測った（候補未登録）。今回の検証: `npm run test:model` 129/129、型検査・lint成功。
